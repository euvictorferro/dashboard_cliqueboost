...
import { getContentFormat } from "@/lib/contentFormat";
            ...
            <div className="space-y-2">
              {groupCards.map((card) => (
                <button
                  ...
                  className="flex w-full items-start gap-3 rounded-[var(--radius-card)] border border-border bg-card p-3 text-left shadow-[var(--shadow-soft)] transition-colors hover:bg-muted"
                >
                  <span
                    className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${
                      getContentFormat(card) === "video" ? "bg-purple-500" : getContentFormat(card) === "text" ? "bg-blue-500" : "bg-brand-primary"
                    }`}
                    aria-hidden="true"
                  />
